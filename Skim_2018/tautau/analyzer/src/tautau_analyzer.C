				  ...
				  if ( MatchTriggerFilter(Tau1Index, Tau2Index) )
				    {
				      ...
				      if(is_MC)
					{
					  if(mvis>20 && mvis<=30) newFF = newFF * 1.8;
					  else if(mvis>30 && mvis<=40) newFF = newFF * 1.05;
					  else if(mvis>40 && mvis<=50) newFF = newFF * 0.85;
					  else if(mvis>50 && mvis<=60) newFF = newFF * 0.65;
					  else if(mvis>60 && mvis<=70) newFF = newFF * 0.6 ;
					  else if(mvis>70 && mvis<=80) newFF = newFF * 0.55 ;
					  else if(mvis>80 && mvis<=90) newFF = newFF * 0.7 ;
					}
					...
					if( pass_bjet_veto )
					  {
						...
						if(tot_tr_mass>50 && tot_tr_mass<290)
      ...
      if(metFilters==0)
      	{
      	  ...
      	  if( passTauTrigger  )
		      ...
		      // 	  <<" "<<subleadingtau<<"  "<<tau_byRawDeepTau2017v2p1VSjet->at(subleadingtau)<<" "<<tau_Charge->at(subleadingtau)<<endl;
				  ...
				  // if(my_njets==0 && !is_MC)
				  //   newFF = newFF * 1.1;
				  //if(is_MC && mvis>20 && mvis<90) cout<<" 20 <  mvis < 90  event_weight ="<<event_weight<<"   newFF = "<<newFF<<endl;
...
double tautau_analyzer::getScaleFactors(  double tau1pt, double tau2pt, double tau1eta, double tau2eta, int taudm, int tau1GenMatch, bool isFakebkg)
{
  ...
  double zpt_weight = get_zptmass_weight();

  // rv_sf = sf_tau1idSF_m * sf_tau2idSF_m * sf_fakeEleMu * t1_trg_pog * t2_trg_pog * higgPt_weight * zpt_weight; 
  
  // if(isFakebkg)
  //   rv_sf = sf_tau1idSF_vvvl * TriggerSF_vvvl.get_sf(tau1dm, tau1pt) * sf_tau2idSF_m * TriggerSF_med.get_sf(tau2dm, tau2pt) * sf_fakeEleMu * higgPt_weight * zpt_weight;

  rv_sf = sf_tau1idSF_m * t1_trg_pog * sf_tau2idSF_m * t2_trg_pog * t1_deeptauid * t2_deeptauid * higgPt_weight * zpt_weight;
  if(isFakebkg)
    rv_sf = sf_tau1idSF_vvvl * sf_tau1idSF_m *  t1_trg_pog * sf_tau2idSF_m * t2_trg_pog * t1_deeptauid * t2_deeptauid *  higgPt_weight * zpt_weight;
  return rv_sf;
