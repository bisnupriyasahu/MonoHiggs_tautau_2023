...
int main(int argc, const char *argv[])
{
  ...
  etau_analyzer t(argv[1], argv[2], isMC, SampleName, signalpara);
...
void etau_analyzer::Loop(Long64_t maxEvents, int reportEvery, string SampleName)
{
  ...
  cout << sample_map[SampleName] << endl;
  // TString trg_file_name = "files_trg/"+sample_map[SampleName];
  // if (is_MC==false)
  //   trg_file_name = "files_trg/SingleElectron.root";
  // TFile *f_eletrg_events = new TFile(trg_file_name);
  // TH1F *h_trg_events_a = (TH1F*)f_eletrg_events->Get("h_trg_events_a");
  // TH1F *h_trg_events_b = (TH1F*)f_eletrg_events->Get("h_trg_events_b");
  ...
  for (Long64_t jentry = 0; jentry < nentriesToCheck; jentry++)
  {
    ...
    // bool passElectronTriggerPaths = false;
    ...
    if (metFilters == 0 && (passMetTrigger==true))
    {
      eventNumber = jentry;
      nSingleTrgPassed += event_weight;
      // if(check_unc)cout<<"Preparing NOMINAL"<<endl;

      // int event_a = event/10000;
      // int event_b = event%10000;
      //cout<<"event : "<<event<< "   "<<event_a<< "   "<<event_b<<endl;
      // if (h_trg_events_a->GetBinContent(event_a)>0 && h_trg_events_b->GetBinContent(event_b)>0)
      //   continue;
  ...
  std::cout << std::setw(20) << std::right << "GoodeleonPassed " << nGoodElectronPassed << std::setw(10) << std::right << "   % change= " << (nSingleTrgPassed - nGoodElectronPassed) * 100 / nSingleTrgPassed << std::endl;
  std::cout << std::setw(20) << std::right << "GoodTauPassed " << nGoodTauPassed << std::setw(10) << std::right << "   % change= " << (nGoodElectronPassed - nGoodTauPassed) * 100 / nGoodElectronPassed << std::endl;
  ...
  std::cout << std::setw(20) << std::right << "opp charge " << nGoodEleTauPassed << std::setw(10) << std::right << "   % change= " << (nGoodTauPassed - nGoodEleTauPassed) * 100 / nGoodTauPassed << std::endl;

  std::cout << std::setw(20) << std::right << "PassedThirdLepVeto " << nPassedThirdLepVeto << std::setw(10) << std::right << "   % change= " << (nGoodEleTauPassed - nPassedThirdLepVeto) * 100 / nGoodEleTauPassed << std::endl;
  ...
  h_cutflow_n->SetBinContent(3, nGoodElectronPassed);
  h_cutflow_n->SetBinContent(4, nGoodTauPassed);
  h_cutflow_n->SetBinContent(5, nGoodEleTauPassed);
