...
OutFile.cd()
print(dirname[0]+"blinded_data_obs_"+histoname)
...
print('ZTT_hist',ZTT_hist.Integral())
print('Wjets_hist',Wjets_hist.Integral())
print('TT_hist',TT_hist.Integral())
print('ST_t_hist',ST_t_hist.Integral())
print('VV_hist',VV_hist.Integral())
print('otherMC_hist',otherMC_hist.Integral())
print('jetFakes_hist',jetFakes_hist.Integral())
...
if 'muPt' in histoname or 'tauPt' in histoname or 'tau1Pt' in histoname or 'tau2Pt' in histoname:
  new_nbins = 3
  Data_hist.Rebin(new_nbins)
  Wjets_hist.Rebin(new_nbins)
  jetFakes_hist.Rebin(new_nbins)
  ZTT_hist.Rebin(new_nbins)
  TT_hist.Rebin(new_nbins)
  ST_t_hist.Rebin(new_nbins)
  VV_hist.Rebin(new_nbins)
  otherMC_hist.Rebin(new_nbins)

if 'higgsPt' in histoname:
  new_binning = array('d', [0, 20, 50, 60, 100, 150, 200, 250, 300, 400])
  new_nbins = 9
  Data_hist = Data_hist.Rebin(new_nbins,  'Data_hist', new_binning )
  Wjets_hist = Wjets_hist.Rebin(new_nbins,  'Wjets_hist', new_binning )
  jetFakes_hist = jetFakes_hist.Rebin(new_nbins,  'jetFakes_hist', new_binning )
  ZTT_hist = ZTT_hist.Rebin(new_nbins,  'ZTT_hist', new_binning )
  TT_hist = TT_hist.Rebin(new_nbins,  'TT_hist', new_binning )
  ST_t_hist = ST_t_hist.Rebin(new_nbins,  'ST_t_hist', new_binning )
  VV_hist = VV_hist.Rebin(new_nbins,  'VV_hist', new_binning )
  otherMC_hist = otherMC_hist.Rebin(new_nbins,  'otherMC_hist', new_binning )
...
if 'tot_TMass_new' in histoname  :
  ...
  new_nbins = 5
  Data_hist = Data_hist.Rebin(new_nbins,  'data_obs', new_binning ) 
  Wjets_hist = Wjets_hist.Rebin(new_nbins,  'WJets_jets', new_binning ) 
  jetFakes_hist = jetFakes_hist.Rebin(new_nbins,  'jetFakes', new_binning ) 
  ZTT_hist = ZTT_hist.Rebin(new_nbins,  'ZTT', new_binning ) 
  TT_hist = TT_hist.Rebin(new_nbins,  'TT', new_binning ) 
  ST_t_hist = ST_t_hist.Rebin(new_nbins,  'ST_t', new_binning ) 
  VV_hist = VV_hist.Rebin(new_nbins,  'VV', new_binning ) 
  otherMC_hist = otherMC_hist.Rebin(new_nbins,  'otherMC', new_binning ) 
  signal_MZp_100_MChi_1 = signal_MZp_100_MChi_1.Rebin(new_nbins,  'signal_MZp_100_MChi_1', new_binning )
  signal_MZp_1000_MChi_1 = signal_MZp_1000_MChi_1.Rebin(new_nbins,  'signal_MZp_1000_MChi_1', new_binning )
  signal_MZp_1500_MChi_1 = signal_MZp_1500_MChi_1.Rebin(new_nbins,  'signal_MZp_1500_MChi_1', new_binning )
...
'''
if histoname=='tot_TMass_full_9' :
  with open('eventYield.csv', mode='a') as yield_file:
    yield_write = csv.writer(yield_file, delimiter=',', quotechar='"')
    yield_write.writerow(['From tau charge, integral, with mT cut' ])
    yield_write.writerow(['Data_hist', Data_hist.Integral() ])
    yield_write.writerow(['ZTT',  ZTT_hist.Integral()  ])
    yield_write.writerow(['Wjets',  Wjets_hist.Integral()  ])
    yield_write.writerow(['TT',   TT_hist.Integral() ])
    yield_write.writerow(['ST',   ST_t_hist.Integral() ])
    yield_write.writerow(['VV',   VV_hist.Integral() ])
    yield_write.writerow(['JetFakes',   jetFakes_hist.Integral() ])
    yield_write.writerow(['otherMC',   otherMC_hist.Integral() ])
    #yield_write.writerow(['ZLL',  ZLL_hist.Integral()  ])                           
    #yield_write.writerow(['Fake', F_bkg.Integral() ])                                                                                                                            
    #    yield_write.writerow(['ggh',  GluGluH_hist.Integral() ])                                                                                                                
    #    yield_write.writerow(['mc sum',  ZTT_hist.Integral()+ZLL_hist.Integral()+F_bkg.Integral()+TT_hist.Integral()+GluGluH_hist.Integral() ])                                     
    yield_write.writerow(['mc sum',  ZTT_hist.Integral()+Wjets_hist.Integral()+TT_hist.Integral()+ST_t_hist.Integral()+VV_hist.Integral()+jetFakes_hist.Integral()+otherMC_hist.Integral() ])
...
label_mapping = {
  ...
  "mT_muMet" : "#tau-met transverse mass [GeV]",
