...
public:
  ...
  TFile * f_mettrg = TFile::Open("sf_files/trg_eff_met120_mht120_2018.root"); 
...
TFile *fw = TFile::Open("sf_files/htt_scalefactors_legacy_2018.root");
  ...
  TLorentzVector my_tau2P4, my_tau1P4, my_metP4;
  TLorentzVector my_tau2P4_nom_5, my_tau1P4_nom_5, my_metP4_nom_5;
  TLorentzVector my_tau2P4_nom_9, my_tau1P4_nom_9, my_metP4_nom_9;
  TLorentzVector my_tau2P4_nom_5dyll, my_tau1P4_nom_5dyll, my_metP4_nom_5dyll;
  TLorentzVector my_tau2P4_nom_9dyll, my_tau1P4_nom_9dyll, my_metP4_nom_9dyll;
  ...
  int Tau1Index_nom, Tau2Index_nom, Tau1Index_nomdyll, Tau2Index_nomdyll;
  ...
  void get_index(vector<int> boostedtau_indices, int &tau1_index, int &tau2_index, bool &isBoosted);
  void get_index_jettau(vector<int> boostedtau_indices, int &tau1_index, int &tau2_index, bool &isBoosted);
...
void tautau_analyzer::setMyEleTau(int tau1Index, int tau2Index, TLorentzVector event_metP4, int shift, bool isBoostedtau)
{
  Tau1Index = tau1Index;
  Tau2Index = tau2Index;

                      
    my_tau1P4.SetPtEtaPhiE(boostedTauPt->at(tau1Index), boostedTauEta->at(tau1Index), boostedTauPhi->at(tau1Index), boostedTauEnergy->at(tau1Index));
    my_tau2P4.SetPtEtaPhiE(boostedTauPt->at(tau2Index), boostedTauEta->at(tau2Index), boostedTauPhi->at(tau2Index), boostedTauEnergy->at(tau2Index));
    
    tau1Isolation = boostedTauByLooseIsolationMVArun2v1DBoldDMwLTNew->at(tau1Index);
    tau1DecayMode = boostedTauDecayMode->at(tau1Index);
    tau1charge = boostedTauCharge->at(tau1Index);
    tau1AntiEle = boostedTauByMVA6LooseElectronRejection->at(tau1Index);
    tau1AntiMu = boostedTauByLooseMuonRejection3->at(tau1Index);    
    
    tau2Isolation = boostedTauByLooseIsolationMVArun2v1DBoldDMwLTNew->at(tau2Index);
    tau2DecayMode = boostedTauDecayMode->at(tau2Index);
    tau2charge = boostedTauCharge->at(tau2Index);
    tau2AntiEle = boostedTauByMVA6LooseElectronRejection->at(tau2Index);
    tau2AntiMu = boostedTauByLooseMuonRejection3->at(tau2Index);
   ...
   if(is_MC){ 
     ...
     corrected_met_v2 = MetRecoilCorrections(Tau1Index, Tau2Index, my_metP4); 
    ...
    my_genmatching_l2 = myGenMaching(my_tau2P4.Eta(), my_tau2P4.Phi());
    pass_bjet_veto = ((bJet_medium().size() == 0) && (bJet_loose().size() < 2));

    deltaR    = my_tau1P4.DeltaR(my_tau2P4);
    deltaPhi  = my_tau1P4.DeltaPhi(my_tau2P4);
    deltaEta  = abs(my_tau1P4.Phi() - my_tau2P4.Phi());
    mT_muMet  = TMass_F( my_tau1P4.Pt(),my_tau1P4.Phi(), my_metP4.Pt(), my_metP4.Phi() );
    visMass_mutau = (my_tau1P4+ my_tau2P4).M();
    HiggsPt   = (my_tau1P4 + my_tau2P4).Pt();
    delpta_phi_HptMet = (my_tau1P4+my_tau2P4).DeltaPhi(my_metP4);
    tot_tr_mass  =  (my_tau1P4 + my_tau2P4 + my_metP4 ).M();
...
void tautau_analyzer::save_nom()
{
  ...
  if (stage == "5_dyll")
  {
    Tau1Index_nomdyll = Tau1Index;
    Tau2Index_nomdyll = Tau2Index;
    l1_genmatching_nomdyll = my_genmatching_l1;
    l2_genmataching_nomdyll = my_genmatching_l2;
    my_tau2P4_nom_5dyll = my_tau2P4;
    my_tau1P4_nom_5dyll = my_tau1P4;
  ...
  if (stage == "9_dyll")
  {
    ...
    l1_genmatching_nomdyll = my_genmatching_l1;
    l2_genmataching_nomdyll = my_genmatching_l2;
    my_tau2P4_nom_9dyll = my_tau2P4;
    my_tau1P4_nom_9dyll = my_tau1P4;
    my_metP4_nom_9dyll = my_metP4;
    my_njets_nom_9dyll = my_njets;
  }
  if (stage == "5")
  {
    Tau1Index_nom = Tau1Index;
    Tau2Index_nom = Tau2Index;
