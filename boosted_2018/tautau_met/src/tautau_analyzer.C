...
void tautau_analyzer::Loop(Long64_t maxEvents, int reportEvery, string SampleName)
{
  ...
  for (Long64_t jentry = 0; jentry < nentriesToCheck; jentry++)
  {
    ...
    ////
    bool passMuonTriggerPaths = false;
    if (HLTEleMuX >> 21 & 1 == 1 || HLTEleMuX >> 60 & 1 == 1 || HLTTau >> 0 & 1 == 1)
      passMuonTriggerPaths = true;
...
void tautau_analyzer::printP4values(string when = "")
{
  ...
  printTabSeparated("entry # : ", eventNumber, "before/after = ", when, "\n",
                    "Shift ", unc_shift, "\n",
                    "electron pt", my_tau1P4.Pt(),
                    "electron eta", my_tau1P4.Eta(), "\n",
                    "tau pt", my_tau2P4.Pt(),
                    "tau eta", my_tau2P4.Eta(), "\n",
                    "MET", my_metP4.Pt(), "\n");
}
