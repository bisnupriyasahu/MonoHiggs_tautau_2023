...
void mutau_analyzer::setMyEleTau(int muIndex, int tauIndex, TLorentzVector event_metP4, int shift, bool isBoostedtau)
{
  ...
  tauAntiMu = boostedTauByLooseMuonRejection3->at(tauIndex);
    ...
    // raw_tauP4.SetPtEtaPhiE(boostedTauPt->at(tauIndex), boostedTauEta->at(tauIndex), boostedTauPhi->at(tauIndex), boostedTauEnergy->at(tauIndex));

    // TLorentzVector corrected_met_v1;
    // corrected_met_v1 = my_metP4 + my_tauP4 - raw_tauP4;

  //  if(is_MC){ 
  //    TLorentzVector corrected_met_v2; 
  //    //corrected_met_v2.SetPtEtaPhiE(pfMET ,0,pfMETPhi,pfMET); 
  //    corrected_met_v2 = MetRecoilCorrections(muIndex, tauIndex, corrected_met_v1); 
