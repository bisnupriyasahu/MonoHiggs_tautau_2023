...
void skimm_signal_2017::Loop(Long64_t maxEvents, int reportEvery, string SampleName, string _isMC_)
{
  ...
  bool debug=false;  double netWeight = 1.0;
   ...
   for (Long64_t jentry=0; jentry<nentriesToCheck;jentry++)
     {
       ...
       cout<<"size of param:  "<<signalParameters->size()<<endl;
            
       if (signalParameters->size() >= 1)
	 {
	   for (std::vector<double>::size_type i = 0; i < signalParameters->size(); i++) {
	     std::cout << "signal param ele : "<< signalParameters->at(i) << ' ';
	     cout<<"type of param :"<<typeid(signalParameters->at(i)).name()<<endl;
	     signalparam  = zprimeBaryonic_signal; 
	     signalparamvec = signalParameters->at(i);
	     //	     if (signalParameters->at(i) == zprimeBaryonic_signal)
	     //
	     if (signalparamvec == signalparam)
	       {
       ...
       //cout<<"signal parameter type after find function  "<<typeid(signalParameters->at(zprimeBaryonic_signal)).name()<<endl;
       //       cout<<"signal_param type after find function  "<<typeid(signalparam).name()<<endl;
   ...
   //   cout<<"nEvents_ZpB is "<<nEvents_ZpB->Integral()<<;
   //cout<<"at end entries in nEvents_ZpB at i : "<<nEvents_ZpB->GetBinContent(zprimeBaryonic_signal)<<std::endl;
   //   cout<<"at end entries in nEvents_ZpB at i+1: "<<nEvents_ZpB->GetBinContent(zprimeBaryonic_signal+1)<<std::endl;
