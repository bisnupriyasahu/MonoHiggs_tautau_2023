						  ...
						  if(higgsPt > 65)
						    {
						      ...
						      if(mvis < 125 && mvis>40)
							{
							  fillHist("8_dyll", Tau1Index, Tau2Index, false, event_weight);
							  if(my_metP4.Pt() > 105)
							    {
							      fillHist("9_dyll", Tau1Index, Tau2Index, false, event_weight);
							    }
							}
...
double tautau_analyzer::getScaleFactors(  double tau1pt, double tau2pt, double tau1eta, double tau2eta, int taudm, int tau1GenMatch, bool isFakebkg)
{
  ...
  // if(my_metP4.Pt()>100){
    // printTabSeparated("\n", "rv_sf = ", rv_sf);
    // printTabSeparated("sf_ :", sf_tau1idSF_m , sf_tau2idSF_m , sf_tauTrg , sf_fakeEleMu );
    // printTabSeparated("t_trg :",  t1_trg_pog , t2_trg_pog  );
    // printTabSeparated("higgsPt, zpt ", higgPt_weight, zptmass_weight);
    // printTabSeparated("TriggerSF_med",  TriggerSF_med.get_sf(tau1dm, tau1pt) , TriggerSF_med.get_sf(tau2dm, tau2pt) );
    // printTabSeparated("TriggerSF_vl", TriggerSF_vvvl.get_sf(tau1dm, tau1pt) , TriggerSF_med.get_sf(tau2dm, tau2pt) );
    // printTabSeparated("isFakebkg:", isFakebkg);
    // printTabSeparated("sd_id_vl", sf_tau1idSF_vvvl );
    // printTabSeparated("t1_deeptauid", t1_deeptauid , t2_deeptauid);
    // cout<<" \n\n"<<endl;
  // }
  ...
  // else
  //   return 1.0;
  rv_sf = sf_tau1idSF_m * sf_tau2idSF_m * TriggerSF_med.get_sf(tau1dm, tau1pt) * TriggerSF_med.get_sf(tau2dm, tau2pt) *  sf_fakeEleMu * t1_trg_pog * t2_trg_pog * higgPt_weight * sf_fakeEleMu;   
  if(isFakebkg)
