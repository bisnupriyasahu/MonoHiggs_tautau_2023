...
bool skimm_mt_2017::skimming_Htt()
{
  ...
  // {
  //   if (tau_Pt->at(iTau) > 29.5 && fabs(tau_Eta->at(iTau)) < 2.3
  //     && tau_byVVVLooseDeepTau2017v2p1VSjet->at(iTau) > 0 )
  //   {
  //     tauFound++;
  //   }
  // }
  for (int iTau = 0; iTau < nBoostedTau; iTau++)
  {
    if (boostedTauPt->at(iTau) > 29.5 && fabs(boostedTauEta->at(iTau)) < 2.3
        && boostedTauByVLooseIsolationMVArun2v1DBoldDMwLTNew->at(iTau) > 0
         )
    {
      boostedtauFound++;
    }
