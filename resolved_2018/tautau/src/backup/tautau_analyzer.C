...
void tautau_analyzer::Loop(Long64_t maxEvents, int reportEvery, string SampleName)
{
  ...
  double hlt1a, hlt2a, hlt3a, hlt4a;
  double hlt1b, hlt2b, hlt3b, hlt4b;
  hlt1=hlt2=hlt3=hlt4=0.0;
  hlt1a=hlt2a=hlt3a=hlt4a=0.0;
  hlt1b=hlt2b=hlt3b=hlt4b=0.0;
  ...
  TH1F* h_trig1=new TH1F("trig1", "trig1", 20, 0, 20); h_trig1->Sumw2();
  TH1F* h_trig2=new TH1F("trig2", "trig2", 20, 0, 20); h_trig2->Sumw2();
		  ...
		  if( tauCand.size() >=2 && tau1>-1 && tau2>-1)
		    {
		      ...
		      //cout<<"entry:"<<jentry<<"            taus picked = "<< tau1 <<"  "<< tau2 <<endl;
		      setMyEleTau(tau1, tau2);
		      
		      //if ( TriggerSelection(my_tau1P4, my_tau2P4) )
		      {
			  
			  //&& (if_DY_Genmatching(Tau1Index, Tau2Index)==1 || if_DY_Genmatching(Tau1Index, Tau2Index)==3) )
			  {
						...
						if(higgsPt > 65)
						  {
							...
							if(my_metP4.Pt() > 105)
							  {
							    fillHist("9", Tau1Index, Tau2Index, false, event_weight);
							    fillUncPlots("9", Tau1Index, Tau2Index, false, event_weight);
							  }
						      }
						  }
					      }
      	  ...
      	  if( passTauTrigger  )
      	    {
      	      ...
      	      tauCand = getTauCand_noID(40.0,2.1);  ///// tau selected
      	      if( tauCand.size() >0 ) 
      		{ 
      		  nGoodMuonPassed_fr+=event_weight;
      		  if(debug)cout<<"this worked Line 526"<<endl;
      		  assignTauIndices(tauCand , leadingtau, subleadingtau);
      		  //makeTestPlot("c", 0,0,0,event_weight);
      		  // int tau1 = -1; int tau2 = -1;
      		  // sortTauIndices(tauCand,tau1, tau2 );
      		  //if( tauCand.size() >=2 && tau1>-1 && tau2>-1)
      		  if( tauCand.size()>=2 && leadingtau>-1 && subleadingtau>-1 )
      		    {
		      // cout<<jentry<<"    "
		      // 	  <<leadingtau<<"  "<<tau_byRawDeepTau2017v2p1VSjet->at(leadingtau) <<" "<<tau_Charge->at(leadingtau)
		      // 	  <<" "<<subleadingtau<<"  "<<tau_byRawDeepTau2017v2p1VSjet->at(subleadingtau)<<" "<<tau_Charge->at(subleadingtau)<<endl;
      				  ...
      				  if ( MatchTriggerFilter(Tau1Index, Tau2Index) )
						  ...
						  if(higgsPt > 65)
						      ...
						      if(mvis < 125)
							{
							  ...
							  if(my_metP4.Pt() > 105)
							    {
							      fillHist("9_fr", Tau1Index, Tau2Index, true, event_weight);
							      fillUncPlots("9_fr", Tau1Index, Tau2Index, true, event_weight);
							    }
							}
						    }
      						}
...
std::vector<int> tautau_analyzer::getTauCand_noID(double tauPtCut, double tauEtaCut){
  ...
  for(int iTau=0;iTau<nTau;iTau++)
	...
	applyTauESCorrections(tauP4, iTau, tauP4_corr);
      ...
      if( tau_DecayMode->at(iTau)!=5 && tau_DecayMode->at(iTau)!=6 )  decayModeCut=true;
  ...
  //   std::cout << iso_map[i].second << " " << iso_map[i].first << " " <<endl;
  tmpCand.clear();
  for(int i=0; i<iso_map.size(); i++)
    tmpCand.push_back( iso_map[i].second );
  //cout<<" tmpcand size = "<<tmpCand.size()<<endl;
  return tmpCand;
...
void tautau_analyzer::sortTauIndices(vector<int> tauIndices , int& tau1 , int& tau2 ){  
  ...
  for(int i=0; i<tauIndices.size(); i++)
    {
      //cout<<" taus "<<i<<"   "<<tau_Charge->at(tauIndices[i])<<"  "<<tau_byRawDeepTau2017v2p1VSjet->at(tauIndices[i])<<endl;
  ...
  if(plus.size()>0 && minus.size()>0)
    {
      ...
      // 	cout<<" plus "<<plus[i].first<<" "<<plus[i].second<<endl;
      // for(int i=0; i<minus.size(); i++)
      // 	cout<<" minus "<<minus[i].first<<" "<<minus[i].second<<endl;
...
void tautau_analyzer::assignTauIndices(vector<int> tauIndices , int& tau1 , int& tau2 ){
  if( tauIndices.size()<2 )
    ...
    {
      // for(int i=0; i<tauIndices.size(); i++)
      // 	cout<<"tau #"<<i<<" "<<tau_byRawDeepTau2017v2p1VSjet->at(tauIndices[i])<<endl;
...
bool tautau_analyzer::TriggerSelection(TLorentzVector tau1P4, TLorentzVector tau2P4){
  ...
  if(is_MC)
    {
      if( HLTTau>>17&1==1 || DoubleTightTauHPS40 )
      	return true;
    }
  else
    {
      //if( HLTTau>>5&1==1 || HLTTau>>6&1==1 || HLTTau>>7&1==1 || HLTTau>>17&1==1 )
      //return true;
      if(DoubleMediumTau40TightID || DoubleTightTau40 || DoubleTightTau35TightID)
	{
	  if (run<317509) return true;
	}
      else if (DoubleTightTauHPS40)
	{
	  if (run>=317509) return true;
	}
    }
...
bool tautau_analyzer::MatchTriggerFilter(int tau1, int tau2)
{
  
  if(!is_MC)  
    return true;
  // tauFiredTrgs and tauFiredL1Trgs
  bool firedtrg = false;
  bool firedL1  = false;
  for(int j=5; j<=17; j++)
    {
      firedtrg = ( tauFiredTrgs->at(tau1)>>j&1==1 || tauFiredTrgs->at(tau2)>>j&1==1);
      firedL1  = ( tauFiredL1Trgs->at(tau1)>>j&1==1 || tauFiredL1Trgs->at(tau2)>>j&1==1) ;
      if(firedL1)
  	return false;
    }
  return true;
...
void tautau_analyzer::fillHist( string histNumber , int tau1Index, int tau2Index, bool isFakeBkg, float event_weight){
  ...
  plotFill("t1Mass_"+hNumber, my_tau1P4.M(),  40, 0, 2.0,  event_weight);
  plotFill("t2Mass_"+hNumber, my_tau2P4.M(),  40, 0, 2.0,  event_weight);
  ...
  //   {
  //     ///// trigger fired
  //     if(tauFiredTrgs->at(tau1Index)>>j&1==1)
  // 	plotFill("t1_trgFired_"+hNumber, j , 20, 0, 20,  event_weight);
  //     if(tauFiredTrgs->at(tau2Index)>>j&1==1)
  // 	plotFill("t2_trgFired_"+hNumber, j , 20, 0, 20,  event_weight);
  //     //// l1 trigger fired
  //     if(tauFiredL1Trgs->at(tau1Index)>>j&1==1)
  // 	plotFill("t1_trgFiredL1_"+hNumber, j , 20, 0, 20,  event_weight);
  //     if(tauFiredL1Trgs->at(tau2Index)>>j&1==1)
  // 	plotFill("t2_trgFiredL1_"+hNumber, j , 20, 0, 20,  event_weight);
  //   }
  
}
void tautau_analyzer::fillUncPlots( string histNumber , int tau1Index, int tau2Index, bool isFakeBkg, float event_weight){
...
void tautau_analyzer::jetFakeUnc( string histNumber , int tau1Index, int tau2Index, bool isFakeBkg, float event_weight){
  ...
  string jetFakesUnc[6] = { "ff_up", "ff_down", "lpt_corr_up", "lpt_corr_down", "mvis_corr_up", "mvis_corr_down"} ;
  string uncNames[6] = {"CMS_rawFF_tt_qcd_2018_up", "CMS_rawFF_tt_qcd_2018_down", "CMS_FF_closure_tau2pt_tt_qcd_up", "CMS_FF_closure_tau2pt_tt_qcd_down", "CMS_FF_closure_tt_qcd_osss_up", "CMS_FF_closure_tt_qcd_osss_down"};
  double mt=TMass_F(my_tau2P4.Pt(),my_tau2P4.Phi()
  		    ,my_metP4.Pt(), my_metP4.Phi());
  double mvis=(my_tau1P4+ my_tau2P4).M();
  ...
  for(int i = 0; i < 6; i++) { 
    double FF_unc = FF_weights_withlpt.get_ff( my_tau1P4.Pt(), mt, mvis
  					       ...
  					       , my_njets, 0
  					       , 0, 0 , 0
  					       , TString(jetFakesUnc[i]));
    //cout<<jetFakesUnc[i] <<" "<< " FF_unc = "<< FF_unc <<endl;
    string histName = histNumber +"_"+ uncNames[i];
    fillHist(histName, Tau1Index, Tau2Index, true, event_weight * FF_unc / FF_nominal);
  }
}
void tautau_analyzer::JESUnc( string histNumber , int tau1Index, int tau2Index, bool isFakeBkg, float event_weight){
  ...
  setMyEleTau( tau1Index, tau2Index);
...
void tautau_analyzer::tauESunc( string histNumber , int tau1Index, int tau2Index, bool isFakeBkg, float event_weight){
  ...
  if (taudm==0) dmName = "1prong";
  else if (taudm==1) dmName = "1prong1pizero";
  else if (taudm==10) dmName = "3prong";
  else if (taudm==11) dmName = "3prong1pizero";
  float tes = get_BinContent(h_tauesSF, taudm); float err = 0;
  if(my_tau1P4.Pt() >= pt_high){
    err = get_BinError(h_tauesSF_highpt, taudm);
  }
  else if (my_tau1P4.Pt() >pt_low ){
    float err_high = get_BinError(h_tauesSF_highpt, taudm);
    float err_low  = get_BinError(h_tauesSF, taudm);
    err = err_low + (err_high-err_low)/(pt_high - pt_low)*(my_tau1P4.Pt() - pt_low);
  }
  else {
    err =get_BinError(h_tauesSF, taudm);
  }
  float weight = event_weight * (tes + err) / tes;
  fillHist(histNumber+"_CMS_scale_t1_"+dmName+"_up", Tau1Index, Tau2Index, true, weight);
  weight = event_weight * (tes - err) / tes;
  fillHist(histNumber+"_CMS_scale_t1_"+dmName+"_down", Tau1Index, Tau2Index, true, weight);
  ...
  tes = get_BinContent(h_tauesSF, taudm); err = 0;
  if(my_tau2P4.Pt() >= pt_high){
    err = get_BinError(h_tauesSF_highpt, taudm);
  }
  else if (my_tau2P4.Pt() >pt_low ){
    float err_high = get_BinError(h_tauesSF_highpt, taudm);
    float err_low  = get_BinError(h_tauesSF, taudm);
    err = err_low + (err_high-err_low)/(pt_high - pt_low)*(my_tau2P4.Pt() - pt_low);
  }
  else {
    err =get_BinError(h_tauesSF, taudm);
  }
  weight = event_weight * (tes + err) / tes;
  fillHist(histNumber+"_CMS_scale_t2_"+dmName+"_up", Tau1Index, Tau2Index, true, weight);
  weight = event_weight * (tes - err) / tes;
  fillHist(histNumber+"_CMS_scale_t2_"+dmName+"_down", Tau1Index, Tau2Index, true, weight);
...
void tautau_analyzer::tauTRGunc( string histNumber , int tau1Index, int tau2Index, bool isFakeBkg, float event_weight){
  ...
  int taudm = tau_DecayMode->at(tau1Index);
  double tauPtCheck= min(450.0, max(20.0, my_tau1P4.Pt()));
  string uncName = "";
  float trg = 1.0; float err = 0;
  trg =  TriggerSF_med.get_sf(taudm, tauPtCheck);
  err = TriggerSF_med.get_sf_err(taudm, tauPtCheck);
  if(taudm==0) uncName = "CMS_doubletautrg_t1_1prong_2018";
  else if(taudm==1) uncName = "CMS_doubletautrg_t1_1prong1pizero_2018";
  else if(taudm==10) uncName = "CMS_doubletautrg_t1_3prong_2018";
  else uncName = "CMS_doubletautrg_t1_3prong1pizero_2018";
  ...
  if(taudm==0) uncName = "CMS_doubletautrg_t2_1prong_2018";
  else if(taudm==1) uncName = "CMS_doubletautrg_t2_1prong1pizero_2018";
  else if(taudm==10) uncName = "CMS_doubletautrg_t2_3prong_2018";
  else uncName = "CMS_doubletautrg_t2_3prong1pizero_2018";
...
void tautau_analyzer::tauIDunc( string histNumber , int tau1Index, int tau2Index, bool isFakeBkg, float event_weight){
  ...
  string uncName = "";
  int taudm = tau_DecayMode->at(tau1Index);
  if (taudm==0)       uncName = "CMS_tauideff_t1_1prong";
  else if (taudm==1)  uncName = "CMS_tauideff_t1_1prong1pizero";
  else if (taudm==10) uncName = "CMS_tauideff_t1_3prong";
  else if (taudm==11) uncName = "CMS_tauideff_t1_3prong1pizero";
  ...
  sf = tauSFTool.getSFvsPT(my_tau2P4.Pt(), my_genmatching_l2);
  sf_up   = tauSFTool.getSFvsPT(my_tau2P4.Pt(), my_genmatching_l2, "Up");
  sf_down = tauSFTool.getSFvsPT(my_tau2P4.Pt(), my_genmatching_l2, "Down");
  taudm = tau_DecayMode->at(tau2Index);
  if (taudm==0)       uncName = "CMS_tauideff_t2_1prong";
  else if (taudm==1)  uncName = "CMS_tauideff_t2_1prong1pizero";
  else if (taudm==10) uncName = "CMS_tauideff_t2_3prong";
  else if (taudm==11) uncName = "CMS_tauideff_t2_3prong1pizero";
...
void tautau_analyzer::metClusteredUnc( string histNumber , int tau1Index, int tau2Index, bool isFakeBkg, float event_weight){
  ...
  resetTauTau( tau1Index, tau2Index);
