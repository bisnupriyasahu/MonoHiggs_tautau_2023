...
def getLabel(name, idx):
    label = name
    label = label.replace('metFull_'+idx, '')
...
def getHistList(fname , channelName, idx):
    ...
    for tdir in sorted(keyList):
        if 'initial' in fname and tdir!='metFull_'+idx:
            continue
        elif ('up' in fname or 'down' in fname) and 'metFull_'+idx+'_' not in tdir:
            continue
        #if 'tot_TMass_full' not in tdir : continue
        #if '_9' not in tdir: continue
        if "_dyll" in tdir: continue
        if tdir == 'metFull_'+idx:
...
file_list = ["f_metFull_initial.root", "f_metFull_up.root", "f_metFull_down.root"]
