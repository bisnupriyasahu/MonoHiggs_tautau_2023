...
void mutau_analyzer::jetFakeUnc( string histNumber , int muIndex, int tauIndex, bool isFakeBkg, float event_weight){
  ...
  for(int u = 0; u < 2; u++) {
    for(int i = 0; i < 15; i++) {
      ...
      //event_weight = event_weight * FF_unc / FF_nominal;
      fillHist(histNumber+"_CMS_"+uncName, MuIndex, TauIndex, true, event_weight* FF_unc / FF_nominal);
    }
  }
}
  ...
  if( my_muP4.Pt()<25.0 && my_tauP4.Pt()>32.0)
    {
      ...
      weight = event_weight * (mu_crosstrg * 1.02) / mu_crosstrg;
      fillHist(histNumber+"_"+uncName+"_up", MuIndex, TauIndex, true, weight);
      weight = event_weight * (mu_crosstrg * 0.98) / mu_crosstrg;
      fillHist(histNumber+"_"+uncName+"_down", MuIndex, TauIndex, true, weight);
    }
  else
    {
      uncName = "CMS_singlemutrg_2018";
      weight = event_weight * (mu_singletrg * 1.02) / mu_singletrg;
      fillHist(histNumber+"_"+uncName+"_up", MuIndex, TauIndex, true, weight);
      weight = event_weight * (mu_singletrg * 0.98) / mu_singletrg;
      fillHist(histNumber+"_"+uncName+"_down", MuIndex, TauIndex, true, weight);
    }
}
...
void mutau_analyzer::electronMissID( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
  ...
  string histName = histNumber + "_CMS_scale_efaket_" + taudm +"_2018";
...
void mutau_analyzer::tauIDunc( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
  ...
  fillHist(histNumber+"_"+uncName+"_up", MuIndex, TauIndex, true, event_weight*sf_up/sf);
  fillHist(histNumber+"_"+uncName+"_down", MuIndex, TauIndex, true, event_weight*sf_down/sf);
...
void mutau_analyzer::fillHist( string histNumber , int muIndex, int tauIndex, bool isFakeBkg, float event_weight){
  ...
  plotFill("metLongXaxis_"+hNumber, my_metP4.Pt() , 30, 100, 250,  event_weight);
