...
std::tuple<double, double> METXYCorr_Met_MetPhi(double originalMet, double originalMet_phi, int runnb, int year, bool isMC, int npv, bool isUL =false,bool ispuppi=false){
  ...
  std::tuple<double, double>  TheXYCorr_Met_MetPhi(originalMet,originalMet_phi);
  //std::cout<<"xyMet correction ..."<<std::endl;
  if(npv>100) npv=100;
  int runera =-1;
  bool usemetv2 =false;
  ...
  //std::cout<<endl;
  //std::cout<<" CorrectedMET "<< CorrectedMET <<" "<< CorrectedMETPhi<<endl;
