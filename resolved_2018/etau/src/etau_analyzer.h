...
public :
   ...
   TH1F* h_nEvents;
   //TH1F* h_nEvents_ZpB=new TH1F("nEvents_ZpB", "nEvents_ZpB", 50, 0, 50);
  ...
  std::vector<std::pair<double, string>> rp_scan_map;
...
void etau_analyzer::Init(TChain *tree, string _isMC_ , string sampleName)
{
  ...
  //h_nEvents_ZpB=new TH1F("nEvents_ZpB", "nEvents_ZpB", 50, 0, 50);
  Ztt_selector=false;
  check_unc=false;
  shift_index=0;
...
void etau_analyzer::make_z_plots(string histNumber, double event_weight){
  int genZCand = -1;
  if(is_MC)
    genZCand = getZCand();
  if(genZCand>-1){
