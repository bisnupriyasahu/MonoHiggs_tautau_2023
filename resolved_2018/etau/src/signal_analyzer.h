...
public :
  ...
  TH2D * h_met_corr = new TH2D("met_corr", "met corrections", 3, 0, 3, 6, 0,  60);
  ...
  float events_with_jetptg50, events_total;
  string unc_shift;
  ...
  // Declaration of leaf types
   ...
   //   signal_analyzer(TTree *tree=0);
   signal_analyzer(const char* file1, const char* file2, string isMC, string SampleName, string year);
   ~signal_analyzer();
   ...
   void make_z_plots(string histNumber, double event_weight);
   void plot_signal(string hnumber);
   //class myclass;
};
#endif

#ifdef signal_analyzer_cxx
signal_analyzer::signal_analyzer(const char* file1, const char* file2, string isMC, string sampleName, string year)
{
...
{
  //std::cout<<"signal_analyzer destructor called"<<std::endl;
  if (!fChain) return;
...
Int_t signal_analyzer::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t signal_analyzer::LoadTree(Long64_t entry)
{
...
void signal_analyzer::Init(TChain *tree, string _isMC_ , string sampleName)
{
...
void signal_analyzer::Show(Long64_t entry)
{
...
Int_t signal_analyzer::Cut(Long64_t entry)
{
...
void signal_analyzer::setMyEleTau(int eleIndex, int tauIndex, TLorentzVector event_metP4, int shift){
...
void signal_analyzer::make_met_plot(string sys_name, string hnumber, float event_weight){
...
void signal_analyzer::make_met_corr_plot()
{
  TLorentzVector raw_met, corrected_met ;
  raw_met.SetPtEtaPhiE(pfMET ,0,pfMETPhi,pfMET);
  corrected_met = MetRecoilCorrections(EleIndex, TauIndex, raw_met);
  ...
  if(Zpt > 50 && num_jets==0) events_with_jetptg50 = events_with_jetptg50 + 1 ;
...
void signal_analyzer::make_z_plots(string histNumber, double event_weight){
  int genZCand = -1;
  if(is_MC)
    genZCand = getZCand();
  if(genZCand>-1){
    plotFill("genZ_Pt_"+histNumber,  mcPt->at(genZCand) , 40 , 0 , 400,  event_weight);
    plotFill("genZ_Mass_"+histNumber,  mcMass->at(genZCand) , 20 , 50 , 150,  event_weight);
  }
}
void signal_analyzer::plot_signal(string hNumber){
  int id_tauplus, id_tauminus;
  id_tauplus = id_tauminus = 0;
  int id_ele, id_mu, id_tau;
  id_ele=id_mu=id_tau=0;
  for(int i=0; i<nMC; i++)
    {
      if(abs(mcPID->at(i))==11 && mcPt->at(i) > 20 && abs(mcEta->at(i)) < 2.3 && abs(mcMotherPID->at(i))==15) id_ele ++;
      if(abs(mcPID->at(i))==13 && mcPt->at(i) > 20 && abs(mcEta->at(i)) < 2.3 && abs(mcMotherPID->at(i))==15) id_mu ++;
      if(abs(mcPID->at(i))==15 && mcPt->at(i) > 20 && abs(mcEta->at(i)) < 2.3 && abs(mcMotherPID->at(i))==15) id_tau ++;
  ...
  //cout<<"FOUND  ele="<<id_ele<< "   mu="<<id_mu <<"   tau=" << id_tau <<endl;
  ...
  plotFill("deltaR_signal_"+hNumber, signal_deltaR , 20, 0, 5,  1.0);

}
#endif // #ifdef signal_analyzer_cxx
