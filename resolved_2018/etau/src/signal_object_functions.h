...
std::vector<int> signal_analyzer::getEleCand(double elePtCut, double eleEtaCut, int shift ){
...
std::vector<int> signal_analyzer::getTauCand(double tauPtCut, double tauEtaCut, int shift){
  std::vector<int> tmpCand;
  tmpCand.clear();
  TLorentzVector dau2;
  //Loop over taus      
...
std::vector<int> signal_analyzer::getAISRTauCand(double tauPtCut, double tauEtaCut, int shift){
...
int signal_analyzer::get_t_Cand()
{
  if(!is_MC)
    return -1;
  for(int iMC=0;iMC<nMC;iMC++) //Loop over mc
...
int signal_analyzer::get_tbar_Cand()
{
  if(!is_MC)
    return -1;
  for(int iMC=0;iMC<nMC;iMC++) //Loop over mc
...
std::vector<int> signal_analyzer::getJetCand(int eleIndex, int tauIndex){
  std::vector<int> tmpCand;  tmpCand.clear();
...
int signal_analyzer::thirdLeptonVeto(){
  std::vector<int> tmpCand;
  tmpCand.clear();
  int thirdLepIndex = -1;
...
bool signal_analyzer::thirdLeptonVeto(int eleIndex, int tauIndex){
  std::vector<int> tmpCand;
  tmpCand.clear();
  int thirdLepIndex = -1;
  bool thirdLepVeto=true;
...
bool signal_analyzer::thirdLeptonVeto(int eleIndex, int tauIndex, int ele2Index){
  std::vector<int> tmpCand;
  tmpCand.clear();
  int thirdLepIndex = -1;
  bool thirdLepVeto=true;
...
double signal_analyzer::delta_R(float phi1, float eta1, float phi2, float eta2)
{
...
double signal_analyzer::DeltaPhi(double phi1, double phi2)
...
float signal_analyzer::TMass_F(float LepPt, float LepPhi , float met, float metPhi) {
...
float signal_analyzer::TotTMass_F(TLorentzVector a, TLorentzVector b, TLorentzVector met) {
  float totalTMass = (a + b+ met).M();
  return totalTMass;
}


float signal_analyzer::VisMass_F(TLorentzVector a, TLorentzVector b){
  float visibleMass = (a + b).M();
  return visibleMass;
}

float signal_analyzer::pTvecsum_F(float pt1, float pt2, float phi1, float phi2) {
  float pt_vecSum = sqrt( pow(pt1*cos(phi1) + pt2*cos(phi2), 2) + pow(pt1*sin(phi1) + pt2*sin(phi2), 2));
  return pt_vecSum;
}
float signal_analyzer::pTvecsum_F(TLorentzVector a, TLorentzVector b, TLorentzVector c) {
  float pt_vecSum = (a + b+ c).Pt();
  return pt_vecSum;
}

vector<int> signal_analyzer::bJet_medium(int eleIndex, int tauIndex)
{
...
vector<int> signal_analyzer::bJet_loose(int eleIndex, int tauIndex)
{
  std::vector<int> tmpJetCand;
  tmpJetCand.clear();
  bool veto = true;
...
bool signal_analyzer::passDiElectronVeto(int eleIndex)
{
...
bool signal_analyzer::eVetoZTTp001dxyz(int eleIndex, int tauIndex){
  std::vector<int> tmpCand;  tmpCand.clear();
...
bool signal_analyzer::mVetoZTTp001dxyz(int eleIndex, int tauIndex){
  std::vector<int> tmpCand;
  tmpCand.clear();
...
int signal_analyzer::myGenMaching(int tauIndex)
{
  if(is_MC==false)
    return 0;
...
int signal_analyzer::myGenMaching1(int eleIndex)
{
  if(is_MC==false)
    return 0;
...
std::vector<int> signal_analyzer::getGenMu(){
  std::vector<int> tmpCand;
  tmpCand.clear();
...
float signal_analyzer::exponential(float x,float a,float b,float c) {
  return a * TMath::Exp(-b * x) + c;
}
double signal_analyzer::exponential( double pT) {
  return TMath::Exp(0.088 - 0.00087*pT + 0.00000092*pow(pT,2) ) ; 
}
double signal_analyzer::getScaleFactors(  double elept, double taupt, double eleeta, double taueta, int taudm, int tauGenMatch, bool isFakebkg)
{
  bool debug=false;
  double rv_sf=1.0;
  ...
  sf_htt_workspace=  e_trk_sf * e_idiso_sf * e_trg24_sf * e_trg_sf * t_deepid_tightvsele_sf * zptmass_weight;
...
bool signal_analyzer::TriggerSelection(TLorentzVector eleP4, TLorentzVector tauP4){
...
bool signal_analyzer::MatchTriggerFilter(int eleIndex, int tauIndex)
{
  
  return true;
}

double  signal_analyzer::getFR(int tauIndex){
  /* double frWeight=1.0; */
  /* double tau_FR = 1.0; */
  /* double tauPt=0.0; */
...
void signal_analyzer::fillUncPlots( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight, int shift){
...
void signal_analyzer::jetFakeUnc( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
...
void signal_analyzer::JESUnc( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
...
void signal_analyzer::JERUnc( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
...
void signal_analyzer::tauESunc( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
...
void signal_analyzer::tauTRGunc( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
...
void signal_analyzer::leptonTRGunc( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
...
void signal_analyzer::electronMissID( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
...
void signal_analyzer::electronES( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
...
void signal_analyzer::muonES( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
...
void signal_analyzer::muonMissID( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
...
void signal_analyzer::tauIDunc( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
...
void signal_analyzer::dyShape( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
...
void signal_analyzer::ttbarShape( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
...
void signal_analyzer::prefiringUnc(string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight)
{
...
void signal_analyzer::metSysUnc( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
...
void signal_analyzer::metClusteredUnc( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
...
float signal_analyzer::EletriggerSF(float pt, float eta){
  float sf = 1.0;
...
void signal_analyzer::makeTestPlot( string histNumber , int eleIndex, int ele2Index, int tauIndex, float event_weight){
  string hNumber = histNumber;
  std::vector<int> tmpCand; tmpCand.clear();
  for(int iEle=0;iEle<nEle;iEle++)
    {
...
TLorentzVector signal_analyzer::MetRecoilCorrections(int eleIndex, int tauIndex, TLorentzVector mymet){
...
TLorentzVector signal_analyzer::applyTauESCorrections(TLorentzVector tauP4, int tauIndex, int shift)
{
...
TLorentzVector signal_analyzer::applyEleESCorrections(TLorentzVector eleP4, int eleIndex, int shift) /*  */
{
...
int signal_analyzer::if_DY_Genmatching(int eleIndex, int tauIndex){
  // if(!is_MC)
  //   return 1;
...
int signal_analyzer::eventCategory(int eleIndex, int tauIndex, double higgsPt){
  int category=0;
  /* std::vector<int> jetCand;       jetCand.clear(); */
  /* jetCand = getJetCand(eleIndex, tauIndex); */
...
double signal_analyzer::getTauFES(int tauIndex, string shift){
...
double signal_analyzer::get_zptmass_weight(){
  double weight = 1.0;
  int genZCand= -1;
  if(is_MC)
    genZCand = getZCand();
...
double signal_analyzer::btag_sf_weight(int eleIndex , int tauIndex){
  double weight = 1.0;
...
double signal_analyzer::eleMuSF(int genmatch, double taueta)
{
