...
histoname=args.name
if args.name=='etau' :
  histoname = 'tot_TMass_full_9'
year_=args.year
category_=args.category
xaxis_label= '_'.join((args.xaxis).split('_')[:-1])
if xaxis_label[-1] == '_':
  xaxis_label = xaxis_label[:-1]
#xaxis_label=args.xaxis[:-3]
if args.name=='etau' :
  xaxis_label = 'etau'
...
if blindingRatio_!=1:
  ...
  #for prefit plots looking at 1/5th of data(means unblinding data to 1/5th)
print(dirname[0]+data_hist_name+histoname)
print(dirname[0]+WJetselect+histoname)
...
if 'tot_TMass_new' in histoname:
  ...
  Data_hist = Data_hist.Rebin(6, 'hist', new_binning )
  Wjets_hist = Wjets_hist.Rebin(6, 'hist', new_binning )
  jetFakes_hist = jetFakes_hist.Rebin(6, 'hist', new_binning )
  ZTT_hist = ZTT_hist.Rebin(6, 'hist', new_binning )
  TT_hist = TT_hist.Rebin(6, 'hist', new_binning )
  ST_t_hist = ST_t_hist.Rebin(6, 'hist', new_binning )
  VV_hist = VV_hist.Rebin(6, 'hist', new_binning )
  otherMC_hist = otherMC_hist.Rebin(6, 'hist', new_binning )
  signal_MZp_100_MChi_1 = signal_MZp_100_MChi_1.Rebin(6, 'hist', new_binning )
  signal_MZp_1000_MChi_1 = signal_MZp_1000_MChi_1.Rebin(6, 'hist', new_binning )
  signal_MZp_1500_MChi_1= signal_MZp_1500_MChi_1.Rebin(6, 'hist', new_binning )
'''


if 'tot_TMass_new' in histoname:
  #new_binning = array('d', [40, 60, 90, 120, 140, 180, 210, 230, 260, 280, 320, 400, 2000]) # 12 binns
...
if 'tot_TMass_new' in histoname:
   ...
   Data_hist.GetXaxis().SetRangeUser(0, 500) 
   Wjets_hist.GetXaxis().SetRangeUser(0, 500) 
   jetFakes_hist.GetXaxis().SetRangeUser(0, 500) 
   ZTT_hist.GetXaxis().SetRangeUser(0, 500) 
   TT_hist.GetXaxis().SetRangeUser(0, 500) 
   ST_t_hist.GetXaxis().SetRangeUser(0, 500) 
   VV_hist.GetXaxis().SetRangeUser(0, 500) 
   otherMC_hist.GetXaxis().SetRangeUser(0, 500) 
  '''
#if False:
#  new_bining = Data_hist.GetNbinsX()
#  Data_hist.Rebin(new_bining)
#  Wjets_hist.Rebin(new_bining)
#  jetFakes_hist.Rebin(new_bining)
#  ZTT_hist.Rebin(new_bining)
#  TT_hist.Rebin(new_bining)
...
#   binErrors = systematics_unc.get_syst(xaxis_label , sampleSyslist[i] , histoname[-2:])
...
#   if syst > 0.12 * content:
#     syst = 0.12 * content
#   final_err = ROOT.TMath.Sqrt( syst**2 + stat**2 )
#   # if final_err < 0.03 * content:
#   #   final_err = prev_err
#   # prev_err = final_err
#   # print ('final_err bin# = {}  , stat = {}  syst = {} , final = {}, content={}'.format(ibin, stat, syst, final_err, content))
...
for ibin in range(1, numberOfBins+1):
  ...
  #syst_6 = 0.1*ST_t_hist.GetBinContent(ibin)
  #syst_7 = 0.1*VV_hist.GetBinContent(ibin)
...
selection_mapping = {
  ...
  "9b" : ["preselection", "+ Higgs pT >65", "+ visible mass<125", "+ met > 105"],
}
...
label_mapping = {
  ...
  "visMass" : "e-#tau visible mass [GeV]",
  "nJet" : "number of Jets",
  "deltaR" : "e-#tau deltaR", 
