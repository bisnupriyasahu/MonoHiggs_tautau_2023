...
public :
  ...
  vector<double>  *signalParameters;
   ...
   signal_analyzer(const char* file1, const char* file2, string isMC, string SampleName, string year, const char* signal_para);
   ~signal_analyzer();
   ...
   void plot_signal(string hnumber, float event_weight);
   //class myclass;
};
#endif

#ifdef signal_analyzer_cxx
signal_analyzer::signal_analyzer(const char* file1, const char* file2, string isMC, string sampleName, string year , const char* signal_para)
{
...
void signal_analyzer::make_met_shapes_plots(string hnumber, double eventWeight){
...
void signal_analyzer::make_met_plots(string hindex){
...
void signal_analyzer::plot_signal(string hNumber, float event_weight){
  ...
  for(int i=0; i<nMC; i++)
    {
      ...
      int mother_id = 25;
      if (found_DYjet_sample==true) mother_id = 23;
      // tau minus
      if(mcPID->at(i)==  15 && mcPt->at(i) > 30 && abs(mcEta->at(i)) < 2.3 && abs(mcMotherPID->at(i))==mother_id)
	id_tauplus = i;
      // taus minus
      if(mcPID->at(i)== -15 && mcPt->at(i) > 30 && abs(mcEta->at(i)) < 2.3 && abs(mcMotherPID->at(i))==mother_id)
	id_tauminus = i;
  ...
  if (found_DYjet_sample==true){
    if (signal_deltaR>0)
      plotFill("deltaR_signal_"+hNumber, signal_deltaR , 60, 0, 3,  event_weight);
  }
  else{
    plotFill("deltaR_signal_"+hNumber, signal_deltaR , 60, 0, 3, event_weight);
  }
