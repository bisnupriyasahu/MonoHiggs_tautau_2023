...
def get_syst(xaxis_label="" , sys_name = None):
    nominal = "TT_inc.root" # "DYJetsToLL_inc.root"
    ...
    #print( norm_hist.GetMaximum(), up1.GetMaximum(), dn1.GetMaximum() )
    
    for y_range in [0.5, 0.2, 0.1, 0.05, 0.02, 0.01]:
        if (abs(up1.GetMaximum()-1) < y_range and abs(dn1.GetMaximum()-1) <y_range):
            norm_hist.SetMaximum(1.0 + y_range)
            norm_hist.SetMinimum(1.0 - y_range)
...
if __name__=="__main__":
    ...
    #histnames = ['elePt_5', 'tauPt_5', 'metLongXaxis_5', 'tot_TMass5']
    histnames = ['nJet', 'met', 'metLongXaxis' , 'visMass', 'higgsPt', 'tot_TMass_full', 'mT_eleMet']
    hnum = ['5', '9']
    for ss in shape_list:
        for h_name in histnames:
            for h_num in hnum:
                get_syst( h_name+'_'+h_num, ss)
