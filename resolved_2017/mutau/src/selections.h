void mutau_analyzer::selections(float weight, int shift, string uncObject)
{
				  ...
				  if(deltaR > 0.5 )
				      ...
				      if( mT_muMet < 50 )
					{
				      ...
				      if(higgsPt > 65)
						...
						if(higgsPt < 65)
						{
						if (shift ==0 ) fillHist("7r_dyll", MuIndex, TauIndex, false, event_weight);
						if(mvis < 125)
							{
							if (shift ==0 ) fillHist("8r_dyll", MuIndex, TauIndex, false, event_weight);
							if(my_metP4.Pt() < 105)
							{
							stage = "9r_dyll";
							if (unc_shift == "nominal" ) save_nom();
							if (shift ==0 ) fillHist("9r_dyll", MuIndex, TauIndex, false, event_weight);
							else fillUncPlots("9r_dyll", MuIndex, TauIndex, false, event_weight, shift);
							}
							}
						}
				    }
  ...
  //muCand = getMuCand(26,2.4, shift);  ///// muons selected 
  if( muCand.size() >0 ) 
    { 
			      ...
			      if( pass_bjet_veto )
				{
				  ...
				  if(deltaR > 0.5 )
				      ...
				      if(higgsPt > 65)
					{
				      ...
				      if(higgsPt < 65)
					{
					  if (shift ==0 ) fillHist("7r", MuIndex, TauIndex, false, event_weight);
					  if(mvis < 125)
					    {
					      if (shift ==0 ) fillHist("8r", MuIndex, TauIndex, false, event_weight);
					      if(my_metP4.Pt() < 105)
						{
						  stage = "9r";
						  if (unc_shift == "nominal" ) save_nom();
						  if (shift ==0 ) fillHist("9r", MuIndex, TauIndex, false, event_weight);
						  else fillUncPlots("9r", MuIndex, TauIndex, false, event_weight, shift);
						}
					    }
					}
				    }
	  ...
	  if ( TriggerSelection(my_muP4, my_tauP4) )
	    {
	      ...
	      bool xtrg = false;
	      //if( HLTTau>>0&1 == 1 && my_muP4.Pt()<=28.0) xtrg=true;
	      if( HLTTau>>0&1 == 1 && my_muP4.Pt()<25.0 && my_muP4.Pt()>21.0) xtrg=true;
			  ...
			  if( thirdLeptonVeto(MuIndex , TauIndex)  )
			      ...
			      if( pass_bjet_veto )
				{
				  ...
				  if(deltaR > 0.5 )
					...
					if(higgsPt < 65)
						{
						if (shift ==0 ) fillHist("7r_fr", MuIndex, TauIndex, false, event_weight);
						if(mvis < 125)
							{
							if (shift ==0 ) fillHist("8r_fr", MuIndex, TauIndex, false, event_weight);
							if(my_metP4.Pt() < 105)
							{
							stage = "9r_fr";
							if (unc_shift == "nominal" ) save_nom();
							if (shift ==0 ) fillHist("9r_fr", MuIndex, TauIndex, false, event_weight);
							else fillUncPlots("9r_fr", MuIndex, TauIndex, false, event_weight, shift);
							}
							}
						}
