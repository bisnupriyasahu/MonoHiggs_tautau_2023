...
public :
   ...
   double nMETFiltersPassed_fr, nPassedSkimmed_fr , nSingleTrgPassed_fr, nGoodMuonPassed_fr, nGoodTauPassed_fr, nGoodMuTauPassed_fr, nPassedThirdLepVeto_fr, nPassedBjetVeto_fr, nDeltaRPassed_fr,nHiggsptPassed_fr,nMVisssPassed_fr,nMETPassed_fr;
   double nMETFiltersPassed, nPassedSkimmed, nSingleTrgPassed, nGoodMuonPassed, nGoodTauPassed, nGoodMuTauPassed, nPassedThirdLepVeto, nPassedBjetVeto, nDeltaRPassed, nHiggsptPassed, nMVisssPassed, nMETPassed, ndatapassedselection, nnominalpassed,njetfakesuppassed,njetfakesdownpassed, nMETRecoilPassed, nGoodTau1Passed, nBeforeTauESPassed, nAfterTauESPassed;
...
void tautau_analyzer::Init(TChain *tree, string _isMC_ , string sampleName)
{
  ...
  nMETFiltersPassed_fr = nPassedSkimmed_fr = nSingleTrgPassed_fr = nGoodMuonPassed_fr = nGoodTauPassed_fr = nGoodMuTauPassed_fr = nPassedThirdLepVeto_fr = nPassedBjetVeto_fr = nDeltaRPassed_fr= nHiggsptPassed_fr = nMVisssPassed_fr = nMETPassed_fr =0;
  nMETFiltersPassed= nPassedSkimmed= nSingleTrgPassed= nGoodMuonPassed= nGoodTauPassed= nGoodMuTauPassed= nPassedThirdLepVeto= nPassedBjetVeto= nDeltaRPassed= nHiggsptPassed = nMVisssPassed=nMETPassed= ndatapassedselection = nnominalpassed=njetfakesuppassed=njetfakesdownpassed = nMETRecoilPassed = nGoodTau1Passed = nBeforeTauESPassed =  nAfterTauESPassed = 0;
...
void tautau_analyzer::setMyEleTau(int tau1Index, int tau2Index, TLorentzVector metP4, int shift, float weight)
{
  ...
  //std::cout<<"before all in set my ele corrected_met is :   "<<corrected_met.Pt()<<endl;
  //cout<<"metP4 pt is : "<<metP4.Pt()<<endl;
  uncorrected_met =  metP4;
  //cout<<"uncorrectedmet pt :  " <<uncorrected_met.Pt()<<endl;
  //cout<<"tau1 met pt :  "<<tmp_tau1.Pt()<< "  & tau2 met pt :  "<<tmp_tau2.Pt()<<endl;
  uncorrectedMetPlusTau=uncorrected_met + tmp_tau1 + tmp_tau2;
  //cout<<"uncorrectedMetPlusTau met pt is:   " <<uncorrectedMetPlusTau.Pt()<<endl;

  if (shift ==0 ) { nBeforeTauESPassed+=event_weight;}
  ...
  if (shift ==0 ) { nAfterTauESPassed+=event_weight;}
  // TLorentzVector corrected_met_v2; 
  //  if(found_DYjet_sample)
 
  //else if (is_MC && found_DYjet_sample == false)
  // corrected_met_v2 = corrected_met;
  //else 
  // corrected_met_v2 = metP4;
  //cout<<"corrected_met_V2 after recoil  :   "<<corrected_met_v2.Pt()<<endl;
  if(is_MC)  corrected_met = uncorrectedMetPlusTau - raw_tau1 - raw_tau2 ;
  /*if(is_MC)
    { 
      TLorentzVector corrected_met_v2; 
  ...
  */
  //cout<<"After all in end setmyele met pt   :   "<<my_metP4.Pt()<<endl;
  if (shift ==0 ) { nMETRecoilPassed+=event_weight;}  
  if(is_MC)
    my_metP4 = corrected_met;
  else
    my_metP4.SetPtEtaPhiE(pfMET ,0,pfMETPhi,pfMET);
