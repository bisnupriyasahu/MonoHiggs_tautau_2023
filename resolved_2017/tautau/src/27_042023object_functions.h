...
double tautau_analyzer::getScaleFactors(  double tau1pt, double tau2pt, double tau1eta, double tau2eta, int taudm, int tau1GenMatch, bool isFakebkg)
{
  ...
  if (found_DYjet_sample)
    {
      rv_sf = sf_tau1idSF_m * sf_tau2idSF_m * TriggerSF_med.get_sf(tau1dm, tau1pt) * TriggerSF_med.get_sf(tau2dm, tau2pt) * sf_fakeEleMu * zptmass_weight ;    
  ...
  if(isFakebkg)
    {
      if (found_DYjet_sample)
	{
	  rv_sf =  sf_tau1idSF_vvvl * sf_tau2idSF_m * TriggerSF_vvvl.get_sf(tau1dm, tau1pt) * TriggerSF_med.get_sf(tau2dm, tau2pt) * sf_fakeEleMu * zptmass_weight;
	  //rv_sf = zptmass_weight;
	}
      else 
	{
...
TLorentzVector tautau_analyzer::MetRecoilCorrections(int tau1Index, int tau2Index, TLorentzVector mymet, int njets){
  ...
  my_njets = njets;
  //  std::cout<<"njets in the met recoil: "<< my_njets<<std::endl;
  // generated Z Boson 
  for(int i=0; i<nMC; i++)
    {
      if(mcPID->at(i)==23)
  ...
  //visGenP4=BosonP4;
  //std::cout<<"bosonpt at zboson before bosonpt === 0 loop : "<<BosonP4.Pt()<<std::endl;
  //int metrec = 0;
  if(BosonP4.Pt()==0)
    {
      for(int i=0; i<nMC; i++)
	  ...
	  if(mcPID->at(i)==-15)
	    {
	      ...
	      //std::cout<<"at i =  "<<i<<"  the gen2 is :  "<<mcPID->at(i)<<"   & gen2 charge  : "<<mcCharge->at(i)<<" & gen2pT "<<gentau2P4.Pt()<<std::endl;
	    }
	}
      std::cout<<"before filling bosonp4 in bosonp4=0 loop gen1 pt : "<<gentau1P4.Pt()<<" & gen2 pT : "<<gentau2P4.Pt()<<std::endl;
      BosonP4.SetPtEtaPhiE(gentau1P4.Pt()+gentau2P4.Pt(), gentau1P4.Eta()+ gentau2P4.Eta(), gentau1P4.Phi()+ gentau2P4.Phi(), gentau1P4.E()+gentau2P4.E());
      //std::cout<<"bosonpt is:  "<<BosonP4.Pt()<<std::endl;
    }
  ...
  /* if(BosonP4.Pt()==0)                                                                                                                                                               
     ...
     for(int i=0; i<nMC; i++)                                                                                                                                                        
     { 	 
     //if(abs(mcPID->at(i))==23  && mcStatusFlag->at(i)>>9&1==1)
	  //std::cout<<"at i =  "<<i<<"  gentau1  : "<<mcPID->at(i)<<"  gentau1 charge:  "<<mcCharge->at(i)<< "   gentau2   :   "<<mcPID->at(i+1)<<"   & gentau2 charge :  "<<mcCharge->at(i+1)<<std::endl; 
	  if(abs(mcMotherPID->at(i))==23 && abs(mcPID->at(i)==15) && abs(mcPID->at(i+1)==15))
	    {
	      //  std::cout<<"in recoil the at i =  "<<i<<"  the gen particle at i :  "<<mcPID->at(i)<<"   & at i+1 is:  "<<mcPID->at(i+1)<<std::endl;
	      gentau1P4.SetPtEtaPhiE(mcPt->at(i), mcEta->at(i) , mcPhi->at(i) , mcE->at(i) );
	      gentau2P4.SetPtEtaPhiE(mcPt->at(i+1), mcEta->at(i+1) , mcPhi->at(i+1) , mcE->at(i+1) ); 
	    }
	}
  ...
  */
  ...
  for(int i=0; i<nMC; i++)
    {
      if((abs(mcPID->at(i))==12 || abs(mcPID->at(i))==14 || abs(mcPID->at(i))==12  ))
	{
	  nuP4tmp.SetPtEtaPhiE(mcPt->at(i), mcEta->at(i) , mcPhi->at(i) , mcE->at(i) );
	  visGenP4=visGenP4-nuP4tmp;
	}
    }
  std::cout<<"bosonpt outside the bosonpt ==0 loop is:  "<<BosonP4.Pt()<<std::endl;
...
TLorentzVector tautau_analyzer::applyTauESCorrections(TLorentzVector tauP4, int tauIndex, int shift)
{
  ...
  //std::cout<<"before tauES  1 correction taupt : "<<tauP4.Pt()<< "   Energy is :  "<<tauP4.E()<<std::endl;
  if (genmatch>=5)
    {
      // std::cout<<" is genmatch >= 5, then tau1 p4 before applying tau es:  "<<tauP4.Pt()<< "   Energy is :  "<<tauP4.E()<<std::endl;
      tauP4 = tauP4 * get_BinContent( h_tauesSF, taudm);
      //std::cout<<" is genmatch >= 5, then tau1 p4 After:  "<<tauP4.Pt()<< "   Energy is :  "<<tauP4.E()<<std::endl;
    }
  else if ( (genmatch==1 || genmatch==3) && taudm==0 )    tauP4 = tauP4*1.003;
  else if ( (genmatch==1 || genmatch==3) && tau2dm==1)    tauP4 = tauP4*1.036;
  //std::cout<<" Final tau1 p4 returning is   "<<tauP4.Pt()<< "   Energy is :  "<<tauP4.E()<<std::endl;
  return tauP4;
}
