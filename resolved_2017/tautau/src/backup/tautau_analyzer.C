		  ...
		  if( tauCand.size() >=2 )
		    {
		      ...
		      // 	tauiso = tau_byIsolationMVArun2017v2DBoldDMwLTraw2017->at(tauCand[i]);
		      // 	cout<<" i="<<i<<" tau:"<<tauCand[i]<<" tauIso="<<tauiso;
		      // }
		      //cout<<" "<<endl;
...
double tautau_analyzer::getScaleFactors(  double tau1pt, double tau2pt, double tau1eta, double tau2eta, int taudm, int tau1GenMatch, bool isFakebkg)
{
  ...
  // if(my_metP4.Pt()>100){
  //   printTabSeparated("\n", "rv_sf = ", rv_sf);
  //   printTabSeparated(sf_tau1idSF_m , sf_tau2idSF_m , sf_tauTrg , sf_fakeEleMu , t1_trg_pog , t2_trg_pog , higgPt_weight, zptmass_weight);
  //   printTabSeparated(isFakebkg);
  //   printTabSeparated(sf_tau1idSF_vvvl , TriggerSF_vvvl.get_sf(tau1dm, tau1pt) , sf_tau2idSF_m , TriggerSF_med.get_sf(tau2dm, tau2pt) , sf_fakeEleMu , higgPt_weight, zptmass_weight);
  //   printTabSeparated( t1_deeptauid , t2_deeptauid);
  // }
...
void tautau_analyzer::jetFakeUnc( string histNumber , int tau1Index, int tau2Index, bool isFakeBkg, float event_weight){
  ...
  string uncNames[6] = {"CMS_rawFF_tt_qcd_2017_up", "CMS_rawFF_tt_qcd_2017_down", "CMS_FF_closure_tau2pt_tt_qcd_up", "CMS_FF_closure_tau2pt_tt_qcd_down", "CMS_FF_closure_tt_qcd_osss_up", "CMS_FF_closure_tt_qcd_osss_down"};
  double mt=TMass_F(my_tau2P4.Pt(),my_tau2P4.Phi()
  		    ,my_metP4.Pt(), my_metP4.Phi());
  double mvis=(my_tau1P4+ my_tau2P4).M();
...
void tautau_analyzer::tauIDunc( string histNumber , int tau1Index, int tau2Index, bool isFakeBkg, float event_weight){
  ...
  for(int i=0; i<4; i++){
    ...
    if(tauDM_map[i].second != taudm) { sf_up = sf; sf_down = sf; }
