...
def get_DYJets_weight():
    ...
    print("LO to NNLO k factor: ", LO_to_NNLO_correction_factor)
    ...
    for sample, lumi in lumi_map.items():
        ...
        if sample=='ZTTjet_inc':
            denominator = lumi 
        print("in sample {}, the nevents/xsec is {} and adding inclusive nevents/xs {} ".format(sample,lumi,lumi_map['ZTTjet_inc']))
        print("sample is {} && the denum is {}".format(sample,denominator))
        ...
        print("final weight (LO_to_NNLO* 41520 )/denum,   of sample {} is {}".format(sample,final_weights[sample]))    
