...
def get_plot(bkg_name, syst_name , filename, inFile):
    ...
    ZTT_Up_err = (ZTT_hist_up - norm_hist)/norm_hist
    ZTT_Down_err = ( norm_hist - ZTT_hist_dn)/norm_hist
    ...
    ZTT_Up_err.SetMarkerStyle(20)
    #ZTT_Up_err.SetMarkerColor(1)
    #ZTT_Up_err.SetLineColor(1)
    #ZTT_ZTT_Up_Err_err.SetFillColor(ROOT.TColor.GetColor(color_ztt))
    ZTT_Up_err.SetMarkerColor(4)
    ZTT_Up_err.SetLineColor(4)
    ZTT_Down_err.SetMarkerColor(2)
    ZTT_Down_err.SetLineColor(2)
    #ZTT_hist.SetLineWidth(3)
    ZTT_Up_err.SetLineWidth(5)
    ZTT_Down_err.SetLineWidth(5)
    ZTT_Up_err.SetStats(0)
    ZTT_Up_err.GetXaxis().SetTitle("")
    ZTT_Up_err.GetYaxis().SetTitle("Events")
    ZTT_Up_err.GetXaxis().SetTitleSize(0)
    ZTT_Up_err.GetXaxis().SetNdivisions(nDivXAxis)
    ZTT_Up_err.GetXaxis().SetLabelSize(0)
    ZTT_Up_err.GetYaxis().SetLabelFont(42)
    ZTT_Up_err.GetYaxis().SetLabelOffset(0.01)
    ZTT_Up_err.GetYaxis().SetLabelSize(0.04)
    ZTT_Up_err.GetYaxis().SetTitleSize(0.05)
    ZTT_Up_err.GetYaxis().SetTitleOffset(1.22)
    if 'MH3' in bkg_name:
        bkg_name = 'signal'
    ZTT_Up_err.SetTitle(bkg_name+ " " + syst_name)

    tmp = None
    tmp = ZTT_Up_err.Clone()
    ...
    ZTT_Up_err.Draw("hist")
    #ZTT_Down_err.Draw("hist same")
    #dn.Draw("histsame")
    #time.sleep(30)
    legende=make_legend()
    #legende.AddEntry(ZTT_hist, 'nominal', "elp")
    legende.AddEntry(ZTT_Up_err, 'shape Up err', "elp")    
    legende.AddEntry(ZTT_Down_err, 'shape Down err', "elp")
    legende.Draw()
    ...
    if not os.path.exists("sys_shape_err_"+channel+"_"+year):
        os.mkdir("sys_shape_err_"+channel+"_"+year)
    c.Modified()
    c.SaveAs("sys_shape_err_"+channel+"_"+year+"/plot_"+bkg_name+'_'+syst_name+".png")
