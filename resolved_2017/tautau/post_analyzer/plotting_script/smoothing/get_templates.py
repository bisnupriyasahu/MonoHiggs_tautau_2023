...
def get_plot(bkg_name, syst_name , filename, inFile):
    ...
    if 'Signal_MZp_500_MChi_1' in bkg_name:
        bkg_name = 'signal'
    ...
    if not os.path.exists("sys_err_"+channel+"_"+year):
        os.mkdir("sys_err_"+channel+"_"+year)
    c.Modified()
    c.SaveAs("sys_err_"+channel+"_"+year+"/plot_"+bkg_name+'_'+syst_name+".png")
...
def get_syst(filename, channel = "tautau"):
    ...
    sampleSyslist = [ 'Signal_MZp_500_MChi_1' ]
    ...
    for thist in keyList:
        ...
        thist = thist.split('_')
        if 'Signal_MZp_500_MChi_1' in thist:
