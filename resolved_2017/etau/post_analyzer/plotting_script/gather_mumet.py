...
def getLabel(name, idx):
    label = name
    label = label.replace('muMetTMassFull_'+idx, '')
...
def getHistList(fname , channelName, idx):
    ...
    for tdir in sorted(keyList):
        if 'initial' in fname and tdir!='muMetTMassFull_'+idx:
            continue
        elif ('up' in fname or 'down' in fname) and 'muMetTMassFull_'+idx+'_' not in tdir:
            continue
        #if 'tot_TMass_full' not in tdir : continue
        #if '_9' not in tdir: continue
        if "_dyll" in tdir: continue
        if tdir == 'muMetTMassFull_'+idx:
...
channelName = 'mutau'#ch
idx = '9'
# inFile_nominal= ROOT.TFile("f_mutau_initial.root","UPDATE")
file_list = ["f_muMetTMassFull_initial.root", "f_muMetTMassFull_up.root", "f_muMetTMassFull_down.root"]
