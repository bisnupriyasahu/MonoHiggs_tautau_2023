...
def make_plot(category="tot_TMass_full", isdata=True):
    ...
    print category+"_9 integral=" , hist_nom_1.Integral(), " --- without Prefiring"
    print category+"_9 integral=" , hist_nom_2.Integral(), " --- with Prefiring" 
    ...
    hist_nom_1.SetTitle(category + " " + filename + " prefiring effects")
    ...
    legende=make_legend()
    legende.AddEntry(hist_nom_1, 'without Prefiring applied', "elp")
    legende.AddEntry(hist_nom_2, 'with Prefiring applied', "elp")    
    ...
    nominal.SetMaximum(1.15)
    nominal.SetMinimum(0.85)
    ...
    c.Modified()
    c.SaveAs(category+"_prefiringeffect.png")
    

if __name__=="__main__":
    make_plot("tot_TMass_full", False)
    make_plot("elePt", False)
    c.Close()
