							  ...
							  if(my_metP4.Pt() > 105)
							    {
							      ...
							      fillUncPlots("9_dyll", EleIndex, TauIndex, false, event_weight);
							    }
							}
							  ...
							  if(my_metP4.Pt() > 105)
							    {
							      ...
							      fillUncPlots("9", EleIndex, TauIndex, false, event_weight);
							    }
							}
		  ...
		  if( tauCand.size()>0 ) 
		    {
		      ...
		      double newFF = FF_weights_withlpt.get_ff( my_tauP4.Pt(), mt, mvis
								...
								, TString(" "));
		      // double newFF_up = FF_weights_withlpt.get_ff( my_tauP4.Pt(), mt, mvis
		      // 						   , 0 , my_eleP4.Pt(), my_metP4.Pt()
		      // 						   , my_njets, xtrg
		      // 						   , frac_tt, frac_qcd, frac_w
		      // 						   , TString("up"));
		      // double newFF_down = FF_weights_withlpt.get_ff( my_tauP4.Pt(), mt, mvis
		      // 						   , 0 , my_eleP4.Pt(), my_metP4.Pt()
		      // 						   , my_njets, xtrg
		      // 						   , frac_tt, frac_qcd, frac_w
		      // 						   , TString("down"));
		      
		      //cout<<newFF<< "  "<<newFF_up<<"  "<<newFF_down<<endl;
							  ...
							  if(my_metP4.Pt() > 105)
							    {
							      ...
							      fillUncPlots("9_fr", EleIndex, TauIndex, true, event_weight);
							    }
							}
...
double etau_analyzer::getScaleFactors(  double elept, double taupt, double eleeta, double taueta, int taudm, int tauGenMatch, bool isFakebkg)
{
  ...
  if( elept<28.0 ) 
    ...
    { e_trg24_sf = 1.0; sf_tauTrg = 1.0; }
    
  sf_htt_workspace=  e_trk_sf * e_idiso_sf *  e_trg24_sf * e_trg_sf;
  rv_sf = sf_tauidSF_m * sf_tauTrg * sf_fakeEle * sf_htt_workspace  *  weight_btagSF * top_pt_weight;
  rv_sf = rv_sf * sf_taufesSF;
  /// uncomment to debug
  // printTabSeparated(elept , eleeta, taupt, taueta , "weights : ");
  // printTabSeparated( e_trk_sf , e_idiso_sf ,  e_trg24_sf , e_trg_sf , zptmass_weight);
  // printTabSeparated( eleEffSF_corr , sf_tauidSF_m , sf_tauTrg , sf_fakeEle , sf_fakeMu  , sf_htt_workspace , higgPt_weight , weight_btagSF , top_pt_weight, sf_tauidSF_vvvl);
...
void etau_analyzer::fillHist( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
  ...
  plotFill("metLongXaxis_"+hNumber, my_metP4.Pt() , 40, 100, 200,  event_weight);
  ...
  float TrMassBins[22]={0.0, 20, 40, 60, 80, 100, 120, 140, 160,180.0, 200.0, 220.0, 240.0,260.0,280.0,300.0,320.0,340.0,360.0,380.0, 400.0, 2000.0};
  double tot_tr_mass = (my_eleP4 + my_tauP4 + my_metP4 ).M();
  plotFill("tot_TMass_"+hNumber, tot_tr_mass , 20, 0, 200,  event_weight);
  plotFill_customBinning("tot_TMass_full_"+hNumber, tot_tr_mass , 21, TrMassBins,  event_weight);
...
void etau_analyzer::fillUncPlots( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
...
void etau_analyzer::jetFakeUnc( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
  ...
  // string jetFakesUnc[22] = {"ff_qcd_up1" , "ff_qcd_down1" , "ff_w_up1", "ff_w_down1" , "ff_tt_up1", "ff_tt_down1" , "lptclosure_xtrg_qcd_up", "lptclosure_xtrg_qcd_down" , "lptclosure_xtrg_w_up", "lptclosure_xtrg_w_down", "lptclosure_xtrg_tt_up", "lptclosure_xtrg_tt_down" , "lptclosure_qcd_up", "lptclosure_qcd_down", "lptclosure_w_up", "lptclosure_w_down", "lptclosure_tt_up", "lptclosure_tt_down", "mtclosure_w_up1", "mtclosure_w_down1", "osssclosure_qcd_down", "osssclosure_qcd_up"};
  double mt=TMass_F(my_eleP4.Pt(),my_eleP4.Phi()
		    ,my_metP4.Pt(), my_metP4.Phi());
  double mvis=(my_eleP4+my_tauP4).M();
  double higgsPt = pTvecsum_F(my_eleP4, my_tauP4, my_metP4);
  ...
  for(int u = 0; u < 2; u++) {
    for(int i = 0; i < 15; i++) {
      ...
      double FF_unc = FF_weights_withlpt.get_ff( my_tauP4.Pt(), mt, mvis
						 ...
						 , frac_tt, frac_qcd, frac_w
						 ,TString(uncName));
      //cout<< uncName <<"  FF_nominal="<<FF_nominal << "    FF_unc = "<< FF_unc <<endl;
      fillHist(histNumber+"_CMS_"+uncName, EleIndex, TauIndex, true, event_weight * FF_unc / FF_nominal);
    }
  }
}
...
void etau_analyzer::tauESunc( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
  ...
  string histName = "CMS_scale_t_"+dmName+"_2017";
  ...
  if( my_eleP4.Pt()<28.0 && my_tauP4.Pt()>35.0)
    {
      ...
      weight = event_weight * (e_trg24_sf * 1.02) / e_trg24_sf;
      fillHist(histNumber+"_"+uncName+"_up", EleIndex, TauIndex, true, weight);
      weight = event_weight * (e_trg24_sf * 0.98) / e_trg24_sf;
      fillHist(histNumber+"_"+uncName+"_down", EleIndex, TauIndex, true, weight);
    }
  else
    {
      uncName = "CMS_singleeletrg_2017";
      weight = event_weight * (e_trg_sf * 1.02) / e_trg_sf;
      fillHist(histNumber+"_"+uncName+"_up", EleIndex, TauIndex, true, weight);
      weight = event_weight * (e_trg_sf * 0.98) / e_trg_sf;
      fillHist(histNumber+"_"+uncName+"_down", EleIndex, TauIndex, true, weight);
    }
}
...
void etau_analyzer::electronMissID( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
  ...
  if (tau_DecayMode->at(tauIndex) == 0 ) taudm = "1prong";
  else if (tau_DecayMode->at(tauIndex) == 1 ) taudm = "1prong1pizero";
  else return;
  ...
  string histName = histNumber + "_CMS_scale_efaket_" + taudm + "_" + region +"_2017";
...
void etau_analyzer::muonES( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
  ...
  string histName = histNumber + "_CMS_scale_m_" +region +"_2017";
...
void etau_analyzer::muonMissID( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
  ...
  string histName = histNumber + "_CMS_scale_mfaket_" + taudm + "_" +region +"_2017";
...
void etau_analyzer::tauIDunc( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
  ...
  if (my_tauP4.Pt() >= 30 && my_tauP4.Pt()<35)
    uncName = "CMS_tauideff_pt30to35_2017";
  else if (my_tauP4.Pt() >= 35 && my_tauP4.Pt()<40)
    uncName = "CMS_tauideff_pt35to40_2017";
  else if (my_tauP4.Pt() >= 40 )
    uncName = "CMS_tauideff_ptgt40_2017";

  fillHist(histNumber+"_"+uncName+"_up", EleIndex, TauIndex, true, event_weight*sf_up/sf);
  fillHist(histNumber+"_"+uncName+"_down", EleIndex, TauIndex, true, event_weight*sf_down/sf);
