				      ...
				      if( thirdLeptonVeto(EleIndex , TauIndex)  )
					{
					  ...
					  if( pass_bjet_veto )
					      ...
					      if(deltaR > 0.5 )
						{
						  ...
						  fillUncPlots("5_dyll", EleIndex, TauIndex, false, event_weight);
							  ...
							  if(my_metP4.Pt() > 105)
							    {
							      ...
							      ////printtabSeparated(__LINE__, "event weight ", event_weight);
				      ...
				      if( thirdLeptonVeto(EleIndex , TauIndex)  )
					{
					  ...
					  if( pass_bjet_veto )
					      ...
					      if(deltaR > 0.5 )
						{
						  ...
						  fillUncPlots("5", EleIndex, TauIndex, false, event_weight);
							  ...
							  if(my_metP4.Pt() > 105)
							    {
							      ...
							      ////printtabSeparated(__LINE__, "event weight ", event_weight);
							      ////printtabSeparated("check pts  after ", my_eleP4.Pt(), my_tauP4.Pt(), my_metP4.Pt());
							    }
							}
				      ...
				      if( thirdLeptonVeto(EleIndex , TauIndex) )
					{
					  ...
					  if( pass_bjet_veto )
					      ...
					      if(deltaR > 0.5 )
						{
						  ...
						  fillUncPlots("5_fr", EleIndex, TauIndex, true, event_weight);
						  double mT_eleMet = TMass_F( my_eleP4.Pt(), my_eleP4.Phi(),
									      my_metP4.Pt(), my_metP4.Phi() );
						  if( mT_eleMet < 50 )
						    {
...
void etau_analyzer::fillUncPlots( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
  ...
  // //printtabSeparated("Prefiring weights ", event_weight, event_weight*prefiringweightup/prefiringweight, event_weight*prefiringweightdown/prefiringweight);
  // pf_tot = pf_tot + 1;
  // if (prefiringweightup == prefiringweightdown ) pf_eq = pf_eq + 1;
  // if (prefiringweightup < prefiringweightdown )  pf_bad = pf_bad + 1;
  // if (prefiringweightup > prefiringweightdown )pf_good = pf_good + 1;
...
void etau_analyzer::electronMissID( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
  ...
  for(int i=0; i<2; i++){
    ...
    if(tauDM_map[i].second != tau_DecayMode->at(tauIndex)) { weight_up = 1.0; weight_down= 1.0;}
    for(int r=0; r<2; r++)
      {
	if (region != regions[r]) { weight_up = 1.0; weight_down= 1.0; }
	string histName = histNumber + "_CMS_scale_efaket_" + taudm + "_" + regions[r] +"_2017";
	fillHist(histName+"_up", EleIndex, TauIndex, true, event_weight * weight_up);
	fillHist(histName+"_down", EleIndex, TauIndex, true, event_weight * weight_down);
      }
  }
...
void etau_analyzer::electronES( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
  ...
  weight = event_weight * (1 - err);
  fillHist(histName+"_down", EleIndex, TauIndex, true, weight);
  //printtabSeparated("_CMS_scale_e_Scale_",  event_weight, event_weight * (1 + err),  event_weight * (1 - err));
...
void etau_analyzer::tauIDunc( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
  ...
  for(int i=0; i<3; i++){
    ...
    fillHist(histNumber+"_"+regions[i]+"_up", EleIndex, TauIndex, true, event_weight*sf_up/sf);
    fillHist(histNumber+"_"+regions[i]+"_down", EleIndex, TauIndex, true, event_weight*sf_down/sf);
  }
...
void etau_analyzer::ttbarShape( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
  ...
  fillHist(histNumber+"_CMS_htt_ttbarShape_up", EleIndex, TauIndex, true, event_weight * (1+err) );
  fillHist(histNumber+"_CMS_htt_ttbarShape_down", EleIndex, TauIndex, true, event_weight * (1-err) );
}
