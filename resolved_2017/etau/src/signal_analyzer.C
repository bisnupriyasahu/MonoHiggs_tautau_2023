...
//$ ./rootcom signal_analyzer analyze
//
...
#include "fractions.C"
//#include "ApplyFF.h"
#include "signal_object_functions.h"
#include "signal_fill_histograms.h"
#include "signal_selections.h"
#include "signal_get_met_systenatics.h"
//#include "myclass.cc"
...
int main(int argc, const char* argv[])
{
  ...
  signal_analyzer t(argv[1],argv[2], isMC, SampleName, year, signalpara);
...
void signal_analyzer::BookHistos(const char* file1, const char* file2, string year)
{
...
void signal_analyzer::fillHistos(int histoNumber, double event_weight, int higgs_Index)
{
...
void signal_analyzer::Loop(Long64_t maxEvents, int reportEvery, string SampleName)
{
...
void signal_analyzer::printP4values(string when=""){
  if(check_unc==false)
    return;
...
float signal_analyzer::average(std::vector<double> const& v){
  if(v.empty()){
    return 0;
  }
