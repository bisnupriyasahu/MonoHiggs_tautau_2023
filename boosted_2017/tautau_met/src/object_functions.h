...
double tautau_analyzer::getScaleFactors(double tau1pt, double tau2pt, double tau1eta, double tau2eta, int tau1dm, int tauGenMatch, bool isFakebkg)
{
  ...
  if(  my_genmatching_l1>=5 )
    {
      sf_tau1idSF_m = get_BinContent( h_tauidSF_m, tau1DecayMode) ;
      sf_tau1idSF_vvvl = get_BinContent( h_tauidSF_vvvl, tau1DecayMode);
    }
  if(  my_genmatching_l2>=5 )
    {
      sf_tau2idSF_m = get_BinContent( h_tauidSF_m, tau2DecayMode) ;
      sf_tau2idSF_vvvl = get_BinContent( h_tauidSF_vvvl, tau2DecayMode);
    }
  ...
  rv_sf = sf_tau1idSF_m * sf_tau2idSF_m * sf_fakeEleMu * higgPt_weight;   
  if(isFakebkg)
    rv_sf = sf_tau1idSF_vvvl * sf_tau2idSF_m ;

  rv_sf = met_trg_sf * rv_sf;
  if(rv_sf>0)
    return rv_sf;
  else
    return 1.0;
...
void tautau_analyzer::jetFakeUnc(string histNumber, int muIndex, int tauIndex, bool isFakeBkg, float event_weight)
{
  ...
  double mt = TMass_F(my_tau1P4.Pt(), my_tau1P4.Phi(), my_metP4.Pt(), my_metP4.Phi());
  ...
  bool xtrg = false;
  if ((HLTTau >> 0 & 1 == 1 || HLTTau >> 14 & 1 == 1) && my_tau1P4.Pt() <= 25.0)
    xtrg = true;
  else if (my_tau1P4.Pt() > 25.0)
    xtrg = false;

  double FF_nominal = FF_weights_withlpt.get_ff(my_tau2P4.Pt(), mt, mvis, 0, my_tau1P4.Pt(), my_tau1P4.Pt(), my_njets, xtrg, frac_tt, frac_qcd, frac_w, TString(" "));
  ...
  for (int i = 0; i < 15; i++)
  {
    ...
    double FF_unc = FF_weights_withlpt.get_ff(my_tau2P4.Pt(), mt, mvis, 0, my_tau1P4.Pt(), my_tau1P4.Pt(), my_njets, xtrg, frac_tt, frac_qcd, frac_w, TString(uncName));
    double weight = event_weight * FF_unc / FF_nominal;
    if (event_weight == 0 || FF_nominal == 0 || FF_unc == 0)
      weight = 0;
    fillHist(histNumber + "_CMS_" + uncName, Tau1Index, Tau2Index, true, weight);
  }
}
